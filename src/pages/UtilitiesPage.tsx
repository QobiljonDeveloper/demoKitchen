...
import { UTILITY_TYPES, UTILITY_UNITS } from '@/api/utilities';
import {
    useUtilities,
    useUtilityConfig,
    useCreateUtility,
...
// ═══════════════════════════════════════════════════════════════
//  FRONTEND FALLBACK RULES  (used when /utilities/config hasn't loaded)
// ═══════════════════════════════════════════════════════════════

const FALLBACK_RULES: Record<UtilityType, UtilityTypeRule> = {
...
function getRuleFor(type: UtilityType, config?: UtilityConfig | null): UtilityTypeRule {
    return config?.rules?.[type] ?? FALLBACK_RULES[type] ?? FALLBACK_RULES.OTHER;
}

function getTypeLabel(type: UtilityType, customLabel: string | null, config?: UtilityConfig | null): string {
    if (type === 'OTHER') return customLabel || 'Boshqa';
    return config?.utilityTypes.find((t) => t.value === type)?.label
        ?? UTILITY_TYPES.find((t) => t.value === type)?.label
        ?? type;
}

function getUnitLabel(unit: UtilityUnit, config?: UtilityConfig | null): string {
    return config?.units.find((u) => u.value === unit)?.label
        ?? UTILITY_UNITS.find((u) => u.value === unit)?.label
        ?? unit;
}

function getAllowedUnitOptions(rule: UtilityTypeRule, config?: UtilityConfig | null) {
    const base = config?.units ?? UTILITY_UNITS;
    return base.filter((u) => rule.allowedUnits.includes(u.value));
}

// ═══════════════════════════════════════════════════════════════
//  ZOD SCHEMA  — base schema, refinements applied at submit
// ═══════════════════════════════════════════════════════════════
...
export function UtilitiesPage() {
    ...
    // ── ref for focusing amount after quick-pick ─────────────
    ...
    // ── queries / mutations ──────────────────────────────────
    const { data: configData, isLoading: isConfigLoading } = useUtilityConfig();
    ...
    const config = (configData ?? null) as UtilityConfig | null;
    ...
    // ── derived rule + allowed units ─────────────────────────
    const currentRule = useMemo(() => getRuleFor(watchType, config), [watchType, config]);

    const allowedUnits = useMemo(
        () => getAllowedUnitOptions(currentRule, config),
        [currentRule, config],
    );
    ...
    // ══════════════════════════════════════════════════════════
    //  AUTO-SET UNIT WHEN TYPE CHANGES  (the critical effect)
    // ══════════════════════════════════════════════════════════
    //  We use a ref to track "last type the effect ran for"
    //  so we don't fight with reset() calls.

    const lastSyncedTypeRef = useRef<string>(watchType);

    useEffect(() => {
        // Only run when type actually changed vs. what we last synced
        if (watchType === lastSyncedTypeRef.current) return;
        lastSyncedTypeRef.current = watchType;

        const rule = getRuleFor(watchType as UtilityType, config);

        // 1) Always set the default unit for the new type
        setValue('unit', rule.defaultUnit, { shouldValidate: true });

        // 2) Clear meters if not supported
        if (!rule.supportsMeter) {
            setValue('meterStart', '', { shouldValidate: false });
            setValue('meterEnd', '', { shouldValidate: false });
        }

        // 3) Clear consumption if not supported
        if (!rule.supportsConsumption) {
            setValue('consumption', '', { shouldValidate: false });
        }

        // 4) Clear custom label if not needed
        if (!rule.requiresCustomLabel) {
            setValue('customTypeLabel', '', { shouldValidate: false });
        }
    }, [watchType, config, setValue]);

    // ── Force-sync unit when only one allowed (single-unit types) ─
    useEffect(() => {
    ...
    const onSubmit = useCallback((data: UtilityFormValues) => {
        ...
        const rule = getRuleFor(type, config);

        // ── client-side rule checks (in addition to zod) ─────
        ...
        if (rule.requiresCustomLabel) {
            payload.customTypeLabel = data.customTypeLabel?.trim();
        }
        if (data.providerName?.trim()) {
            payload.providerName = data.providerName.trim();
        }
        ...
        if (rule.supportsMeter) {
            ...
            payload.meterEnd = me;
            // backend computes consumption from meters, so only send manual if no meters
            if (ms != null && me != null) {
    ...
    }, [config, editingId, createMutation, updateMutation, reset]);
    ...
    const openNewForm = useCallback((type: UtilityType = 'ELECTRICITY') => {
        const rule = getRuleFor(type, config);
        setEditingId(null);
        lastSyncedTypeRef.current = type; // prevent effect from fighting reset
        reset({
            date: format(new Date(), 'yyyy-MM-dd'),
            utilityType: type,
            customTypeLabel: '',
        ...
        // Focus amount input after dialog opens
    ...
    const handleEdit = useCallback((row: UtilityTransaction) => {
        ...
        lastSyncedTypeRef.current = row.utilityType; // prevent effect
        reset({
    ...
    const columns = useMemo<ColumnDef<UtilityTransaction>[]>(
        () => [
            ...
            },
            {
                ...
                cell: ({ row }) => (
                    <div className="font-medium">
                        {getTypeLabel(row.original.utilityType, row.original.customTypeLabel, config)}
                    </div>
                ),
            },
            {
            ...
            },
            {
                ...
                cell: ({ row }) => {
                    ...
                    return val != null
                        ? `${val} ${getUnitLabel(row.original.unit, config)}`
                        : '—';
                },
            },
            {
        ...
        [handleEdit, handleDeleteClick, config],
    );
    ...
    const utilityTypes = config?.utilityTypes ?? UTILITY_TYPES;
    ...
    return (
        <div className="space-y-6">
            ...
            {/* ═══════════════════════════════════════════════════
                 CREATE / EDIT DIALOG  —  dynamic form
                ═══════════════════════════════════════════════════ */}
                ...
                <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
                    ...
                    {isConfigLoading ? (
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                                    ...
                                    <Controller
                                        ...
                                        render={({ field }) => (
                                            <Select
                                                ...
                                                onValueChange={(val) => {
                                                    field.onChange(val);
                                                    // effect will handle unit sync via useEffect
                                                }}
                                            >
                            ...
                            {currentRule.requiresCustomLabel && (
                                <div className="space-y-2">
                                    ...
                                    <Controller
                                        ...
                                        render={({ field }) => (
                                            <Input
                                                id="util-custom-label"
                                                value={field.value || ''}
                                                onChange={field.onChange}
                                                onBlur={field.onBlur}
                                                ref={field.ref}
                                                placeholder="Masalan: Parking, Lift, Domkom..."
                                            />
                                        )}
                                    />
                                    ...
                                    <Controller
                                        ...
                                        render={({ field }) => (
                                            <Input
                                                ...
                                                onBlur={field.onBlur}
                                                ref={field.ref}
                                                placeholder="Masalan: UzbekEnergo, Hududgaz..."
                                            />
                                        )}
                                    />
                                </div>
                            ...
                            {/* ── Unit  (FULLY CONTROLLED) ──── */}
                            <div className="space-y-2">
                                <Label htmlFor="util-unit">O'lchov birligi *</Label>
                                {isSingleUnit ? (
                                    /* Single allowed unit → read-only display */
                                    <Input
                                        id="util-unit"
                                        value={allowedUnits[0].label}
                                        readOnly
                                        tabIndex={-1}
                                        className="bg-muted cursor-default"
                                        aria-label={`O'lchov birligi: ${allowedUnits[0].label}`}
                                    />
                                ) : (
                                    /* Multiple allowed units → Select dropdown */
                                    <Controller
                                        control={control}
                                        name="unit"
                                        render={({ field }) => (
                                ...
                                <>
                                    ...
                                    {currentRule.meterRequired && (
                                        <p className="text-xs text-muted-foreground -mt-2">
                                            {getTypeLabel(watchType, null, config)} uchun meter ko'rsatkichlari majburiy
                                        </p>
                                    )}
                                </>
                            )}
                                    ...
                                    render={({ field }) => (
                                        <MoneyInput
                                            ...
                                            getInputRef={(el: HTMLInputElement | null) => {
                                                // Wire both react-hook-form ref and our focus ref
